# Image README

This file has a list of the image assets and relevant details.

* dancing1.png - From Rocky Mountain Rendezvous September 2024
  - not on website because I don't know who to give photo credit to
* headshot1.png - taken by a friend in Santa Fe, NM March 2024
* headshot2.png - taken by my sibling on the Ring of Kerry in Ireland May 2024
* headshot3.png - selfie taken after a haircut in August 2024

Icons and logos I created using canva
* icon-dancer.svg 
* logo.png

Taken from the fernfolio template
* icon-hamburger.svg
* icon-close.svg
* icon-sun.svg